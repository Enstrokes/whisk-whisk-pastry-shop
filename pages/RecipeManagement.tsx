...
const RecipeManagement: React.FC = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      if (!stockRes.ok) {
        throw new Error(`Stock API failed with status: ${stockRes.status}`);
      }
      ...
      // Process recipes
      const processedRecipes = recipesData.map((r: any) => ({ ...r, id: r._id || r.id }));
      setRecipes(processedRecipes);

      // Process stock items
      const processedStockItems = (stockData.results || []).map((s: any) => ({ 
        ...s, 
      ...
      console.error('[RecipeManagement] Error fetching data:', e);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button 
            onClick={() => { 
              setSelectedRecipe({ name: '', ingredients: [], sellingPrice: 0 }); 
              setFormModalOpen(true); 
            }} 
            className="w-full sm:w-auto bg-brand-primary hover:bg-brand-secondary text-white font-bold py-2 px-4 rounded-lg transition-colors flex items-center justify-center gap-2"
          >
            + Create Recipe
          </button>
        </div>
              ...
              <tr key={recipe.id} className="border-t border-brand-border">
                ...
                <td className="p-4">
                  {recipe.ingredients.map((ing, idx) => {
                    ...
                    return stock ? (
                      <span key={idx}>
                        {stock.name} ({ing.quantity} {stock.unit})
                ...
                <td className="p-4 text-right">
                  <button 
                    ...
                    className="p-2 text-brand-text-secondary hover:text-yellow-400 mr-2" 
                    aria-label="Edit"
                  >
                    Edit
                  </button>
                  <button 
                    ...
                    className="p-2 text-brand-text-secondary hover:text-red-500" 
                    aria-label="Delete"
                  >
                    Delete
                  </button>
      ...
      <Modal 
        ...
        title={selectedRecipe && 'id' in selectedRecipe ? 'Edit Recipe' : 'Create Recipe'} 
        size="4xl"
      >
        {selectedRecipe && (
          <RecipeForm
